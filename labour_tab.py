...
class LabourTab(ft.Container):
    def __init__(self, page: ft.Page):
        ...
        self.l_id = ft.TextField(label="آئی ڈی (ID)", width=150, value=self.get_next_id(), read_only=True, text_style=ft.TextStyle(size=22, weight=ft.FontWeight.BOLD, font_family=URDU_FONT, color=ft.Colors.BLUE_900), label_style=ft.TextStyle(size=18, weight=ft.FontWeight.BOLD, font_family=URDU_FONT))
        self.l_name = ft.TextField(label="مزدور کا نام (Name)", width=300, text_style=ft.TextStyle(size=22, weight=ft.FontWeight.BOLD, font_family=URDU_FONT), label_style=ft.TextStyle(size=18, weight=ft.FontWeight.BOLD, font_family=URDU_FONT), on_submit=self.add_labour)
        
        add_btn = ft.ElevatedButton(
            "مزدور شامل کریں / محفوظ کریں", 
            on_click=self.add_labour, 
            bgcolor=ft.Colors.BLUE_600, color=ft.Colors.WHITE, 
            style=ft.ButtonStyle(padding=15, text_style=ft.TextStyle(font_family=URDU_FONT, size=20, weight=ft.FontWeight.BOLD))
        )

        self.labour_list = ft.Column(spacing=15)

        self.content = ft.Column([
            ft.Row([
                self.error_text,
                ft.Text("مزدوروں کا انتظام (Manage Labour)", size=30, weight=ft.FontWeight.BOLD, text_align=ft.TextAlign.RIGHT, font_family=URDU_FONT, color=ft.Colors.BLUE_900),
    ...
    def refresh_dropdowns(self): pass
    ...
    def add_labour(self, e):
        if not self.l_name.value: return
        existing = next((l for l in data_store.labourers if l.get('id') == self.l_id.value), None)
        if existing: existing['name'] = self.l_name.value
        else: data_store.labourers.append({"id": self.l_id.value, "name": self.l_name.value, "transactions": []})
    ...
    def delete_labour(self, lab):
        ...
        for i, l in enumerate(data_store.labourers): l['id'] = str(i + 1)
        data_store.save_data()
        self.l_id.value = self.get_next_id()
        self.load_visual_list()
        self.main_page.update()
    ...
    def load_visual_list(self):
        ...
        for i, l in enumerate(data_store.labourers):
            ...
            khata_btn = ft.ElevatedButton(
                ...
                style=ft.ButtonStyle(padding=15, text_style=ft.TextStyle(font_family=URDU_FONT, size=18, weight=ft.FontWeight.BOLD)),
            ...
            row = ft.Row([
                del_btn, edit_btn, 
                ft.Container(content=khata_btn, width=280), 
                ft.Container(content=ft.Text(f"فیکٹری بقایا: {prod_balance:,.0f}", weight=ft.FontWeight.BOLD, size=22, color=ft.Colors.ORANGE_800, font_family=URDU_FONT), width=220),
                ft.Container(expand=True), 
                ft.Container(content=ft.Text(f"نام: {l['name']}", weight=ft.FontWeight.BOLD, size=26, color=ft.Colors.BLUE_900, font_family=URDU_FONT, text_align=ft.TextAlign.RIGHT), width=250),
                ft.Container(content=ft.Text(f"آئی ڈی: {l.get('id', str(i+1))}", weight=ft.FontWeight.BOLD, size=20, font_family=URDU_FONT, text_align=ft.TextAlign.RIGHT), width=80),
            ], vertical_alignment=ft.CrossAxisAlignment.CENTER)
            
            self.labour_list.controls.append(ft.Container(content=row, padding=20, border=ft.border.all(1, ft.Colors.BLUE_GREY_200), border_radius=8, bgcolor=ft.Colors.WHITE))
    ...
    def open_khata(self, labour):
        ...
        self.t_amount = ft.TextField(label="رقم (Amount)", width=150, keyboard_type=ft.KeyboardType.NUMBER, dense=True, text_style=ft.TextStyle(font_family=URDU_FONT, size=24, weight=ft.FontWeight.BOLD))
        self.t_reason = ft.TextField(label="تفصیل (Reason)", expand=True, dense=True, text_style=ft.TextStyle(font_family=URDU_FONT, size=22))
        
        self.btn_in = ft.ElevatedButton("رقم جمع کی (Deposit)", icon=ft.Icons.ADD_CIRCLE, bgcolor=ft.Colors.GREEN_600, color=ft.Colors.WHITE, style=ft.ButtonStyle(padding=15, text_style=ft.TextStyle(font_family=URDU_FONT, size=18, weight=ft.FontWeight.BOLD)), on_click=lambda e: self.add_transaction("IN"))
        self.btn_out = ft.ElevatedButton("ایڈوانس دیا (Advance)", icon=ft.Icons.REMOVE_CIRCLE, bgcolor=ft.Colors.RED_600, color=ft.Colors.WHITE, style=ft.ButtonStyle(padding=15, text_style=ft.TextStyle(font_family=URDU_FONT, size=18, weight=ft.FontWeight.BOLD)), on_click=lambda e: self.add_transaction("OUT"))
        
        self.t_list = ft.ListView(height=350, spacing=15) 
        self.t_total = ft.Text("ذاتی بیلنس: 0", size=30, weight=ft.FontWeight.BOLD, font_family=URDU_FONT)
        ...
        try:
            if self.active_dialog and self.active_dialog in self.main_page.overlay: self.main_page.overlay.remove(self.active_dialog)
            self.active_dialog = ft.AlertDialog(title=ft.Text(f"{labour['name']} کا ذاتی لین دین", size=30, weight=ft.FontWeight.BOLD, font_family=URDU_FONT, color=ft.Colors.BLUE_900, text_align=ft.TextAlign.RIGHT), content=content_container, actions=[ft.ElevatedButton("بند کریں (Close)", icon=ft.Icons.CLOSE, color=ft.Colors.RED_900, bgcolor=ft.Colors.RED_50, on_click=self.close_khata)], actions_alignment=ft.MainAxisAlignment.END)
    ...
    def add_transaction(self, t_type):
        ...
        if 'transactions' not in self.current_labour: self.current_labour['transactions'] = []
        self.current_labour['transactions'].append({"id": str(uuid.uuid4()), "date": date_str, "amount": amt, "type": t_type, "reason": reason})
        data_store.save_data() 
        self.t_amount.value = ""; self.t_reason.value = ""
        self.refresh_khata_list()
        self.main_page.update()
    ...
    def refresh_khata_list(self):
        ...
        for t in reversed(transactions):
            ...
            row = ft.Container(
                content=ft.Row([
                    del_btn, ft.Text(t['date'], size=16, color=ft.Colors.GREY_600, font_family=URDU_FONT),
                    ft.Container(expand=True), ft.Text(t['reason'], size=20, font_family=URDU_FONT, color=ft.Colors.BLACK, weight=ft.FontWeight.BOLD),
                    ft.Row([ft.Text(f"{sign}{t['amount']:,.0f}", size=24, weight=ft.FontWeight.BOLD, color=color, font_family=URDU_FONT), ft.Icon(icon, color=color, size=24)], spacing=5)
                ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                padding=15, border_radius=8, bgcolor=bg_color, border=ft.border.all(1, color), margin=ft.margin.only(bottom=5)
            )
